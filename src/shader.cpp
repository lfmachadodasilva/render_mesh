...
bool cShader::create(void)
{
  ...
  m_color_loc    = glGetUniformLocation(m_program_id, "global_color");
  m_has_pick_loc = glGetUniformLocation(m_program_id, "has_pick");
...
// -------------------------------------------------------------------------------------------------
// -------------------------------------------------------------------------------------------------
void cShader::load(const sRenderAtt &render_att)
{
  glUseProgram(m_program_id);

  // need to send to shader the model view matrix without object trasnformation
  // to transform the light position to the eye space
  glUniformMatrix4fv( g_shader->m_projection_matrix_loc, 1, GL_FALSE, 
                      render_att.model_view_matrix.GetMatrix());

  glUniform3fv(m_light_position_loc, 1, render_att.position_light);
  
  glUniform4fv(m_ambient_light_loc,  1, g_light_att.ambient_light);
  glUniform4fv(m_diffuse_light_loc,  1, g_light_att.diffuse_light);
  glUniform4fv(m_specular_light_loc, 1, g_light_att.specular_light);
}
