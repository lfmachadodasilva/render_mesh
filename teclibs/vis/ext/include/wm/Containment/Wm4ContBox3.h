...
// If fails, returns the fail point coeficient in *fail_coef (if != NULL) and
// the fail point extents in *fail_extents (if != NULL).
template <class Real> WM4_FOUNDATION_ITEM
bool InBox (const Vector3<Real>& rkPoint, const Box3<Real>& rkBox, Real* fail_coef, Real* fail_extents);
...
// Construct an oriented box that contains a number of other oriented boxes.
// The result is not guaranteed to be the minimum volume box containing the
// input boxes.
template <class Real> WM4_FOUNDATION_ITEM
Box3<Real> MergeBoxes (int numBoxes, const Box3<Real>* rkBoxes);
