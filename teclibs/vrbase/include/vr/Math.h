...
//! \file math.h Common Math functions and constants.
...
/*!
    \defgroup common_math Common Math Functions and Constants
    @{
*/

//! Common arithmetic constants.
//! This is a template class; you may prefer to use vr::Mathf or vr::Mathd instead.
...
public:
	...
	static VRBASE_EXPORT const real NAN_VALUE;		//!< Standard invalid value.
};
...
template<typename real>
inline bool isEqual( real a, real b, real tolerance = Math<real>::ZERO_TOLERANCE )
{
	real delta = b - a;
...
// -- works as a no-op for unsigned types
template<> inline uint8 abs<uint8>( uint8 a ) { return a; }
template<> inline uint16 abs<uint16>( uint16 a ) { return a; }
template<> inline uint32 abs<uint32>( uint32 a ) { return a; }
template<> inline uint64 abs<uint64>( uint64 a ) { return a; }
//@}
...
//! Computes the cubic root of value.
template<typename real>
inline real cbrt( real value )
{
	return value > 0.0 ? pow( value, (real)1.0/3.0 ) : ( value < 0 ? -pow( -value, (real)1.0/3.0 ) : 0 );
}

//! Contains common Math constants using float precision.
typedef Math<float> Mathf;

//! Contains common Math constants using double precision.
typedef Math<double> Mathd;

/*! @} */ // end of group common_math
